...
class Song:
    ...
    def check_if_song_exists(self) -> bool:
        query=f"SELECT yt_link FROM songs WHERE user=\'{self.yt_link}\'"
        result=connect_execute_query(query)
        if result:
            logging.warning(f"Song [{result[0][0]}] you wanted to add already exists.")
            return False
        else:
            return True
    ...
    def input_song(self) -> bool:
        if self.check_if_song_exists():
            logging.info(f"Adding [{self.song_name}] song into db.")

            template = load_db_template(defaults.song_insert)
            insert_query = template.render(song_name=self.song_name, artist=self.aritst, genre=self.genre, yt_link=self.yt_link)
